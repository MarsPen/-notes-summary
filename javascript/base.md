...
### 闭包
  1.说起闭包那么首先先来了解一下js执行环境（execution context）、作用域以及作用域链（scope chain）
   - 执行环境是js的一个重要概念，**执行环境**定义了变量或者函数有权访问的其他数据，决定了各自的行为。
   - 当前执行环境中定义的变量和函数都保存在与之关联的**变量对象**中（varible object）
   - 当代码在执行环境中运行时，会创建**变量对象**的一个**作用域链（词法作用域）**，能够保证对执行环境中所有变量和函数有序访问
